jQuery.entwine("citacroppable", function($) {
    ...
    $("input.croppableimage").entwine({
        ...
        onmatch: function() {
            var self = this;
            this.setDialog(self.siblings('.cita-croppable-dialog:first'));
            ...
            if (self.val().length){
                url = url + '?CitaCroppableImageID=' + self.val();
            }else{
                url = url + '?CitaCroppableImageID=0';
            }
        ...
        onunmatch: function () {
            var self = this;
            $('.cita-croppable-dialog.ui-dialog-content').filter(function(){
        ...
        onclick: function(e) {
            ...
            if(this.is('.ss-uploadfield-item-delete')) {
                if(confirm(ss.i18n._t('UploadField.ConfirmDelete'))) {
                    this.parents('fieldset:eq(0)').find('.cita-cropper').hide();
                    if (fileupload) {
    ...
    $('.cita-croppable-dialog .uploadfield__upload-button').entwine(
    {
        onmatch :   function(e)
                    {
